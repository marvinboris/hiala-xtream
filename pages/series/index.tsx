...
const SeriesPage: NextPageWithLayout = () => {
  ...
  const seriesStreamsContent = data !== null && data.filter(stream => categoryId === 0 || stream.category_id == categoryId).filter(stream => stream.title.toLowerCase().includes(search.toLowerCase())).map(renderSeriesStream)
