...
const BouquetPage: NextPageWithLayout = () => {
  ...
  const data = rawData && "id" in rawData && rawData;
  ...
  const channelsContent =
    ...
    [...liveCategories]
      ...
      .map((category) => {
        const category_channels = data.channels
          .filter((channel) => channel.category_id === category.id)
          .map((channel) => (
            <div
              key={`channel-${channel.id}`}
              className="rounded-2xl bg-white shadow-sm flex items-center space-x-2"
            >
              <div className="aspect-square w-16 rounded-2xl rounded-r-none relative p-4 flex items-center justify-center overflow-hidden z-0">
                <img
                  src={assets(channel.stream_icon)}
                  alt={channel.stream_display_name}
                  className="w-full h-full object-contain"
                />
                <img
                  src={assets(channel.stream_icon)}
                  alt={channel.stream_display_name}
                  className="w-full h-full object-cover absolute inset-0 -z-10 blur-3xl scale-150"
                />
              </div>

              <div
                className="truncate text-secondary-700"
                title={channel.stream_display_name}
              >
                {capitalize(channel.stream_display_name.toLocaleLowerCase())}
              </div>
            </div>
          ));

        return category_channels.length > 0 ? (
          <div key={`bouquet-page-live-category-${category.id}`}>
        ...
        <main>
          <header className="container flex items-center space-x-5">
            ...
            {data && (
              <div>
                <View
                  title={`Souscrire au bouquet ${data.bouquet_name}`}
                  action={<Button color="primary">Souscrire</Button>}
                >
                  <BouquetSubscribe
                    amount={
                      data.bouquet_name.toLowerCase() === "passion"
                        ? 3000
                        : data.bouquet_name.toLowerCase() === "magic"
                        ? 5000
                        : data.bouquet_name.toLowerCase() === "charme"
                        ? 2000
                        : data.bouquet_name.toLowerCase() === "canal+"
                        ? 12000
                        : 2
                    }
                    name={data.bouquet_name}
                    id={data.id}
                  />
                </View>
              </div>
