...
const BouquetsPage: NextPageWithLayout = () => {
  ...
  const maxBouquetNumber =
    data &&
    "length" in data &&
    Math.max(...data.map((bouquet) => bouquet.bouquet_channels.length));
  ...
  const bouquetsContent = data && "length" in data && data.map(renderBouquet);
        ...
        <main className="relative overflow-hidden z-0">
          ...
          {status === Status.FAILED ? (
            ...
            >
              ...
              <div className="container hidden md:grid gap-6 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {bouquetsContent}
              </div>
            </section>
          )}
