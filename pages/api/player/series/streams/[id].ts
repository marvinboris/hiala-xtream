...
export default async function handler(
    ...
    try {
        ...
        const episode = await Stream.findByPk(+id)
        const source = await axios.get<string>(`${process.env.XTREAM_HOSTNAME!}/series/${username}/${password}/${id}.${episode!.target_container[0]}`)
