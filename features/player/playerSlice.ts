...
// Lives
export const liveCategories = createAsyncThunk(
  "player/live/categories",
  async () => await getLiveCategories()
);
export const liveStreams = createAsyncThunk(
  "player/live/streams",
  async (category_id?: number) => await getLiveStreams(category_id)
);

// Series
export const seriesCategories = createAsyncThunk(
  "player/series/categories",
  async () => await getSeriesCategories()
);
export const seriesStreams = createAsyncThunk(
  "player/series/streams",
  async (category_id?: number) => await getSeriesStreams(category_id)
);
export const seriesInfo = createAsyncThunk(
  "player/series/info",
  async (series_id: number) => await getSeriesInfo(series_id)
);

// Vod
export const vodCategories = createAsyncThunk(
  "player/vod/categories",
  async () => await getVodCategories()
);
export const vodStreams = createAsyncThunk(
  "player/vod/streams",
  async (category_id?: number) => await getVodStreams(category_id)
);
export const vodInfo = createAsyncThunk(
  "player/vod/info",
  async (vod_id: number) => await getVodInfo(vod_id)
);

// Bouquets
export const bouquets = createAsyncThunk(
  "player/bouquets",
  async (id?: number) => await getBouquets(id)
);

const dataPending =
  (
    category: "live" | "series" | "vod" | "bouquets",
    list?: "categories" | "streams" | "info"
  ) =>
  (state: any) => {
    if (list) {
...
const dataFulfilled =
  ...
  (state: any, action: any) => {
    if (list) {
      ...
      if (!(category === "live" && list === "info"))
        state[category][list].data = action.payload;
    } else {
...
export const playerSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        seriesCategories.fulfilled,
        dataFulfilled("series", "categories")
      )
