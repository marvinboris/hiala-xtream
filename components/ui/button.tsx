...
export default function Button({ icon: Icon, block, disabled, color = 'primary', ...rest }: ButtonProps) {
    return <button {...rest} className={classNames(
        ...
        color === 'white' ? disabled ? "bg-secondary-200" : "text-primary-600 bg-white hover:bg-secondary-200" : "text-white",
        color === "primary" ? disabled ? "bg-primary-800" : "bg-primary-600 hover:bg-primary-800" : "",
        rest.className || ''
    )}>
        {Icon && <div className={classNames('relative mr-[14px]', color === 'link' ? "" : "pr-[14px]")}>
            <Icon className={classNames('w-[18px]', color === 'primary' ? 'text-white/80' : 'text-primary-600')} />
