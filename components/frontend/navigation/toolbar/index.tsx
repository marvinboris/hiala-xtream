...
export default function Toolbar({ white = false }) {
  ...
  const navItems: NavItemType[] = account
    ? [
  ...
  const navItemsContent = navItems.map((item) => (
    <NavItem key={`nav-item-${item.href}`} {...item} />
  ));

  return (
    <Popover
      className={classNames(
        "sticky w-full top-0 z-30 border-b border-secondary-500/50",
        white ? "bg-white text-secondary-600" : "bg-secondary-700 text-white"
      )}
    >
      {({ close }) => (
        <>
            ...
            <div className="pt-9 pb-9 lg:pt-12">
              <div className="flex items-center justify-between">
                ...
                <div className="ml-auto flex items-center space-x-14">
                  <Popover.Group
                    as="nav"
                    className="hidden space-x-[22px] md:flex"
                  >
                    {navItemsContent}
                  </Popover.Group>
                ...
                <div className={!account ? "" : "ml-8 lg:ml-14"}>
                  {account ? (
                    <Account navItems={navItems} />
                  ) : (
                    <div className="relative group md:hidden">
                      <Squares2X2Icon
                        className={classNames("w-8", white ? "text-teal" : "")}
                      />
                      ...
                      <div className="lg:flex items-center justify-center lg:space-x-5 absolute lg:static top-full right-0 mt-1 bg-white lg:bg-transparent py-2 rounded-lg scale-0 group-hover:scale-100 origin-top-right transition-all duration-200">
                        ...
                        {!account ? (
                          <NavLink href="/auth/login">Se connecter</NavLink>
                        ) : null}
                      </div>
                    </div>
                  )}
