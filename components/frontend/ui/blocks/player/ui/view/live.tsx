...
export default function LiveView({ category, info }: LiveViewProps) {
    ...
    const similar = info !== null ? available.filter(s => s.id !== info.id) : []
    ...
    return <>
        <div className="bg-primary-800/5 lg:bg-white lg:border-b border-secondary-600/20 lg:sticky top-0">
            <div className="h-[90px] lg:h-[135px] container flex items-center">
                <div>
                    <div className="font-bold text-[25px]">Chaînes TV ({available.length})</div>
        ...
        <div className="container pt-8 lg:pt-[22px] pb-[94px] lg:pb-[58px] overflow-auto">
            ...
            <div className="grid gap-x-3.5 gap-y-2 grid-flow-col grid-rows-6 pb-4">
                {similar.map(s => <Link key={s.slug} href={`/chaines/${category?.slug}/${s.slug}`} shallow>
                    <a className="min-w-[237px] rounded-[14px] bg-secondary-200/70 p-2 space-x-3 flex">
                        <img src={assets(s.stream_icon)} alt={s.stream_display_name} className="w-[62px] h-[70px] flex-none rounded-[10px] bg-white p-2" />
                        <div className="flex-1 truncate text-xs pt-2">
                            <div className="text-sm truncate font-bold capitalize">{s.stream_display_name}</div>
                            <div className='capitalize'>{categories?.find(c => c.id === s.category_id)?.category_name.toLowerCase()}</div>
                            <div className='mt-1'>
                                <span className="rounded-full leading-none px-3 py-0.5 text-xs text-white bg-red-600">En direct</span>
                            </div>
                        </div>
                    </a>
                </Link>)}
