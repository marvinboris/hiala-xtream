...
const formatTime = (duration: number) => (
  <>
    {Math.floor(duration / 60 / 60) ? `${Math.floor(duration / 60 / 60)}h` : ""}
...
export default function SerieView({ info }: SerieViewProps) {
  ...
  const seasons =
    data
      ?.map((episode) => episode.season_num)
      .filter(onlyUnique)
      .sort() || [];
  ...
  const totalSeconds =
    data?.reduce(
      (a, b) =>
        a +
        (b.stream.movie_propeties.duration_secs ||
    ...
    <>
        ...
        <div className="h-[123px] lg:h-[135px] container flex lg:items-center pt-[22px] lg:pt-0">
          ...
          <div className="space-y-2 w-full hidden lg:block">
            ...
            <div className="bg-secondary-600/10 rounded-lg px-6">
              <select
                className="h-11 w-full outline-none hidden lg:block bg-transparent text-secondary-600/30"
                value={season}
          ...
          <div key={`season-${s}`}>
            <button
              ...
              className={classNames(
                "h-11 w-[116px] flex items-center justify-center text-sm",
                season === s
      ...
      <div className="container pt-8 lg:pt-[22px] pb-[94px] lg:pb-[58px] overflow-auto z-0">
        <div className="grid gap-y-2.5 lg:gap-y-3.5">
          {data
            ?.filter((e) => e.season_num === season && e.id !== info.id)
            .map((episode) => {
              const series = streams.data?.find(
                (series) => series.id === episode.series_id
              )!;
              const category = categories?.find(
                (category) => category.id === series.category_id
              )!;

              return (
                <Link
                  key={episode.stream.slug}
                  href={`/series/${category?.slug}/${series.slug}/${episode.stream.slug}`}
                  shallow
                >
                  <a className="rounded-[18px] lg:rounded-lg bg-secondary-200/70 lg:bg-black/40 lg:text-white p-2.5 space-x-3.5 flex relative overflow-hidden h-[120px]">
                    <img
                      src={assets(series.cover)}
                      alt={episode.stream.stream_display_name}
                      className="w-[82px] h-[92px] lg:absolute inset-0 lg:w-full lg:h-full lg:-z-10 flex-none object-cover rounded-[13px] lg:rounded-lg"
                    />
                    <div className="flex-1 truncate lg:hidden">
                      <div className="text-lg truncate font-bold capitalize">
                        Episode {episode.sort}
                      </div>
                      <div className="my-1 w-[3px] h-[3px] rounded-full bg-red-400" />
                      <div>
                        Titre :{" "}
                        <span className="font-bold">
                          {episode.stream.stream_display_name}
                        </span>
                      </div>
                      <div>
                        Durée :{" "}
                        <span className="font-bold">
                          {formatTime(
                            episode.stream.movie_propeties.duration_secs ||
                              series.episode_run_time ||
                              0
                          )}
                        </span>
                    ...
                    <div className="hidden lg:flex text-lg items-center justify-center w-full h-full">
                      <div>
                        <span className="font-bold text-white">
                          Saison {episode.season_num} - EP
                          {episode.sort < 10 ? 0 : null}
                          {episode.sort}
                        </span>
                      </div>
