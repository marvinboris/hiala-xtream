...
export default function View({ stream, action }: ViewProps) {
  ...
  const condition =
    !account ||
    !account.bouquet ||
    !account.bouquet.find(
      (bouquet) =>
        (type === "stream" &&
          bouquet.bouquet_channels.find((item) => item === stream.id)) ||
        (type === "serie" &&
          bouquet.bouquet_series.find((item) => item === stream.id))
    );
    ...
    <div>
      ...
      <Transition show={isOpen} as={Fragment}>
        <Dialog
          ...
          className="fixed inset-0 z-50 flex md:items-center"
        >
              ...
              <div className="-mx-4 md:mx-0">
                <nav className="h-12 pr-1.5 flex items-center">
                  <XMarkIcon
                    ...
                    className="cursor-pointer w-9 md:w-11 ml-auto text-white hover:text-white/50 transition-all duration-200"
                  />
                </nav>
                  ...
                  <div className="relative aspect-video bg-secondary-800">
                    <img
                      src={assets(
                        "cover" in stream
                          ? stream.cover
                          : stream.movie_propeties.cover_big
                      )}
                      alt={name}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center flex-col z-10 px-5 lg:px-10 text-center bg-secondary-900/50 text-secondary-100">
                      {condition ? (
                        <>
                          <div className="hidden md:block text-3xl font-bold mb-6">
                            Envie de regarder {name}?
                          </div>

                          <div className="text-sm md:text-lg font-bold mb-8">
                            Découvrez nos bouquets du moment.
                          </div>
                        ...
                        <div>
                          {type === "stream" ? (
                            <Link
                              href={`/films/${category!.slug}/${stream.slug}`}
                            >
                              <a className="btn btn-primary">
                                Regarder maintenant
                              </a>
                  ...
                  <div className="p-4 md:p-6 text-lg">
                    <div className="text-3xl md:text-4xl font-medium text-white mb-3">
                      <Link
                        href={
                          "cover" in stream
                            ? `/series/${category?.slug}/${stream.slug}`
                            : `/films/${category?.slug}/${stream.slug}`
                        }
                      >
                        <a>{name}</a>
                      </Link>
                    ...
                    {stream &&
                      ("seasons" in stream ? (
                        <ViewSeries series={stream} category={category} />
                      ) : (
                        <div>
                          <div className="mb-5 capitalize">
                            {stream.movie_propeties.genre},{" "}
                            {stream.movie_propeties.releasedate}
                          </div>
