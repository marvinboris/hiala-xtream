...
const EpisodePlot = ({ episodes, info, category }: Plot) => (
  <div className="space-y-4">
    {episodes.map((episode) => (
      <Link
        key={`series-info-episode-${episode.id}`}
        href={`/series/${category.slug}/${info.slug}/${episode.stream.slug}`}
      >
        <a className="flex">
          <div className="mr-2 w-1/4 hidden md:block">
            <div className="relative aspect-video bg-secondary-800">
              <img
                src={assets(
                  episode.stream.movie_propeties.movie_image || info.cover
                )}
          ...
          <div className="flex-1">
            <div className="text-white mb-4">
              <div className="mt-2 font-semibold md:font-normal text-base md:text-lg">
                Épisode {episode.sort}
              </div>
            ...
            <div className="text-base">
              {episode.stream.movie_propeties.plot}
            </div>
          </div>
        </a>
      </Link>
...
const NoEpisodePlot = ({ episodes, info, category }: Plot) => (
  <div className="grid gap-x-2 gap-y-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
    {episodes.map((episode) => (
      <Link
        key={`series-info-episode-${episode.id}`}
        href={`/series/${category.slug}/${info.slug}/${episode.stream.slug}`}
      >
        <a className="block">
          ...
          <div className="mt-2 text-white text-sm md:text-lg">
            Épisode {episode.sort}
          </div>
...
export default function ViewSeries({
  ...
  const episodes = allEpisodes.filter(
    (episode) => episode.season_num === season
  );
  ...
  const seasonsContent =
    ...
    seasons.map((s, index) => (
      <div
        key={`series-info-season-${index}`}
        onClick={() => setSeason(s)}
        className={classNames(
          "h-10 w-[56px] flex items-center justify-center text-white cursor-pointer",
          s === season ? "bg-primary-800" : "bg-secondary-700"
        )}
      >
        {s}
      </div>
  ...
  return (
    <>
      ...
      <div>
        <div className="md:flex justify-between mb-3">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <Switch
                checked={episodePlot}
                toggle={() => setEpisodePlot(!episodePlot)}
              />
            </div>
          </div>
        </div>
        ...
        {episodePlot ? (
          <EpisodePlot episodes={episodes} info={info} category={category} />
        ) : (
          <NoEpisodePlot episodes={episodes} info={info} category={category} />
        )}
      </div>
    </>
  );
