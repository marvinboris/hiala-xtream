...
export function VideoJS({ live, info, category, children }: VideoProps) {
  ...
  if (!condition) {
    ...
    ).stream_source.map((src) => ({
      src,
      type: live ? "application/x-mpegURL" : "video/webm",
    }));
        ...
        <div className="h-screen flex flex-col">
          <header className="container flex items-center h-20 lg:h-[133px]">
            <div className="mr-6 md:mr-12">
              <div
                onClick={back}
                className="cursor-pointer w-12 h-12 rounded-full flex items-center justify-center bg-white/30 text-white"
              >
            ...
            {live ? (
              <img
                ...
                alt="Stream Icon"
                className="h-12 object-center mr-3"
              />
            ) : null}
            <div>
            ...
            <div className="ml-auto">
              {live ? children : <HeartIcon className="w-14 text-white" />}
            </div>
          </header>
